function [V, C, topo] = T1_edgeswap(V, C, topo, T1_edge_ID)
% T1_EDGESWAP  Perform a T1 (neighbor-exchange) topological transition on
% a single edge (non-sheared PBC version).
%
%   [V, C, topo] = T1_EDGESWAP(V, C, topo, T1_edge_ID) collapses the edge
%   T1_edge_ID (connecting vertices u0 and v0, shared by cells c1 and c2)
%   and re-expands it perpendicular to its original orientation, so that
%   the two cells that used to touch along this edge (c1, c2) separate,
%   and the two cells that used to only meet at a point (cu, cv) become
%   new neighbors. This is the standard T1 transition used to model
%   cell-neighbor exchange / tissue rearrangement in vertex models.
%
% Inputs:
...
% actually realizes the neighbor exchange geometrically).
xv0 = V(u0,1) + pbc_dist(V(v0,1) - V(u0,1), box_size);
yv0 = V(u0,2) + pbc_dist(V(v0,2) - V(u0,2), box_size);
xu0 = V(u0,1);
yu0 = V(u0,2);
xmid = 0.5 * (xv0 + xu0);
ymid = 0.5 * (yv0 + yu0);
